...
total.plot = ggplot(pkmn.df, aes(x = total, y = catch_rate)) + 
  geom_point() +
...
The dataset is split into 75:25 training to test dataset. We choose Boosting, Random Forest, Bagging, and Support Vector Machine models to train the data with cross validation. RMSE is used to compare and choose the final model. 
...
Support Vector Machine with both linear and radial kernel is used to train the data. 
...
## 3.3 Final Model and Visualization
