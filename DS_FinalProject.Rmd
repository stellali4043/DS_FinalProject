...
featurePlot(x = trainData[, 2:7], y = trainData$catch_rate,
            plot = "scatter",
            span = .5,
...
pred.rf <- predict(rf.fit, newdata = pkmn.df[-trRows,])
...
# fast implementation
set.seed(2021)
rf2 <- ranger(catch_rate ~ .,
              pkmn.df[trRows,],
              mtry = 6) 

pred.rf2 <- predict(rf2, data = pkmn.df[-trRows,])$predictions
...
```{r}
resamp <- resamples(list(rf = rf.fit, rf2 = rf2, gbm = gbm.fit, svmr = svmr.fit))
summary(resamp)
```
