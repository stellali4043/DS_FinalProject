...
bagging.rmse = RMSE(rf.pred, pkmn.df$catch_rate[trRows])
bagging.rmse
...
```{r message=FALSE, warning=FALSE}
# Grid search using `caret`, and use the fast implementation of random forest when tuning the model.
...
```{r message=FALSE, warning=FALSE}
set.seed(2021)
vip(rf.fit,method = "permute",train = pkmn.df[trRows,],
...
final.model.table = data.frame(model = rep(c("Boosting", "Random Forest", "Bagging", "SVM (linear)", "SVM (radial)")),
                               RMSE = rep(c(gbm.rmse, rf.rmse, rf.rmse2, svml.rmse, svmr.rmse))) %>%
  knitr::kable()
final.model.table %>%
